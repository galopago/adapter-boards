# Barrel Panel 3p 5.5mmx2.1mm to Screw 3mm5 adapter PCB

This board was created to convert a barrel panel connector (3 pin, 5.5mm x 2.1mm) to screw terminals (3mm5 pitch).

![PCB 3D picture](/barrel-panel-3p-5mm5x2mm1-to-screw-3mm5/assets/img/pcbadapter.jpg)


PCB FRONT                                          | PCB BACK
---------------------------------------------------|----------------------------------------------
![](/barrel-panel-3p-5mm5x2mm1-to-screw-3mm5/assets/img/pcbfront.jpg) |![](/barrel-panel-3p-5mm5x2mm1-to-screw-3mm5/assets/img/pcbback.jpg) 


## Panelized PCB
![](/barrel-panel-3p-5mm5x2mm1-to-screw-3mm5/assets/img/pcbpanel.jpg)

## How to use this repository

The PCB was developed in KiCad V6.0.

## Directory structure

* The root folder contains KiCad files: project, schematic and PCB.
* gerber/single directory contains ready to manufacture files, for a single board.
* gerber/panel directory contains ready to manufacture files that fits in a panel (use Vcuts!).
* assets directory some additional info about the project.

## License

[![Creative Commons License](https://i.creativecommons.org/l/by/4.0/88x31.png)](http://creativecommons.org/licenses/by/4.0/)

This is an Open Hardware project and is licensed under a [Creative Commons Attribution 4.0 International License](http://creativecommons.org/licenses/by/4.0/).

